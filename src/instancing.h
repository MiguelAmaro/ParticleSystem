...
instancing InstancingInit(d3d11_base *Base)
{
  ...
  D3D11ConstantBuffer(Device, &Result.Consts, NULL, sizeof(instancing_consts), Usage_Dynamic, Access_Write);
  ArenaTempEnd(Temp);
  ...
  str8 ShaderFile = Str8("F:\\Dev\\ParticleSystem\\src\\instancing.hlsl");
  Result.Reset      = D3D11ShaderCreate(ShaderKind_Compute, ShaderFile, Str8("KernelReset"), NULL, 0, Base);
  Result.Instancing = D3D11ShaderCreate(ShaderKind_Compute, ShaderFile, Str8("KernelInstancing"), NULL, 0, Base);
